...
install_mod_assemblies() {
	...
	if [ "${RUNTIME}" = "mono" ]; then
		...
		find . -maxdepth 1 -name '*.sln' -exec dotnet publish -c Release -p:TargetPlatform="${TARGETPLATFORM}" -p:PublishTrimmed=false -r "${TARGETPLATFORM}" -o "${DEST_PATH}" --self-contained true \;
		cd "${ORIG_PWD}" || exit 1
	fi
}
